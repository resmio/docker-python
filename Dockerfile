...
RUN apt-get update -qq && apt-get upgrade -y && apt-get install -y build-essential python3.8 python3.8-dev python3-distutils libpq-dev ca-certificates git libxml2-dev libxslt-dev gettext binutils libproj-dev gdal-bin libffi-dev

# Use newer pip
RUN curl https://bootstrap.pypa.io/get-pip.py | python3.8 - "pip==20.0.2"
